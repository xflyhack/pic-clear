...
# ----------------------------- 长路径兼容 -----------------------------------
# helper 实现搬到 winpath_util.py 集中维护 (v0.4.61 起).
# 这里保留下划线别名, 让下面的业务代码零改动.
from winpath_util import (
    normalize_windows_path as _normalize_windows_path,
    to_long_path as _to_long_path,
    resolve_mapped_drive_to_unc as _resolve_mapped_drive_to_unc,
    pil_diag as _pil_diag,
    pil_open as _pil_open,
...
def main() -> int:
    ...
    # v0.4.82: --version 走"早期短路", 不能先跑 _check_license_or_die,
    # 否则 [授权] xxx 会跑到 stdout 第一行, GUI 关于页把它误当成版本号显示.
    if any(a in ("--version", "-V") for a in sys.argv[1:]):
        print(f"dedupe_pic {_read_version()}")
        return 0
